...
class SparseBEVHead(DETRHead):
    ...
    def trim_points(self, global_points ,npoints=900) :
       
        indices = torch.randint(0, global_points.shape[1], (npoints,))
        selected_points = global_points[0,indices]  # shape: [900, 3]
        sorted_points, sorted_indices = torch.sort(selected_points, dim=0)
        # 결과를 [1, 900, 3] 형태로 변형
        sorted_points = sorted_points.unsqueeze(0)  # 차원 추가하여 [1, 900, 3] 형태로 변환

        return sorted_points
    
    def map_points_to_grid(self,trim_points, grid):
        # trim_points의 shape: [batch, 14430, 3]
        # grid의 shape: [900, 2]
        
        batch_size = trim_points.shape[0]
        num_points= trim_points.shape[1]
        # trim_points를 (batch, 3, H, W) 형태로 변환
        # 여기서 H와 W는 임의로 정할 수 있지만, grid에 맞춰야 합니다.
        # 예를 들어 H와 W를 120으로 설정한다고 가정합시다.
        H, W = 30*4, 30*4  # H와 W는 필요에 따라 조정하세요.
        
        # trim_points를 (batch, 3, H, W) 형태로 변환
        # 이 과정은 실제로는 3D 포인트 클라우드를 2D 형태로 변형하는 과정입니다.
        # 이를 위해 선형 보간을 사용하여 적절한 텐서로 변환해야 합니다.
        trim_points = trim_points.reshape(batch_size, 2, H, W)  # [batch, 3, H, W]
        
        # grid를 (1, 900, 1, 2) 형태로 변환
        grid = grid.unsqueeze(0).unsqueeze(2)  # [1, 900, 1, 2]
        grid = grid * 2 - 1  # [0, 1] -> [-1, 1]로 변환
        
        # grid_sample을 사용하여 trim_points 맵핑
        # trim_points는 (batch, 2, H, W) 형태여야 하므로, 임의의 H와 W로 변형합니다.
        output = F.grid_sample(trim_points, grid, mode='bilinear', padding_mode='border')
    ...
    def forward(self, pts_feats ,mlvl_feats, img_metas):
        ...
        # query_bbox_raw[:,:2] = self.calc_bbox_position(mapping_points.permute(0,2,1))
        ...
        # query_feat_pc = self.pointNet_contents(global_points)
