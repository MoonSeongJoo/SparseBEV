...
class SparseBEV(MVXTwoStageDetector):
    ...
    def extract_pts_feat(self, pts, img_feats, img_metas):
        ...
        if self.with_pts_neck:
            x = self.pts_neck(x)
        # 모든 요소를 torch.float32로 변환
        x = [feat.to(torch.float32) for feat in x]
        return x
    ...
    def forward_test(self, img_metas, img=None, points=None, points_raw =None, points_gt=None,points_mis=None,global_points=None,z_points=None, **kwargs):
        for var, name in [(img_metas, 'img_metas')]:
            if not isinstance(var, list):
                raise TypeError('{} must be a list, but got {}'.format(
                    name, type(var)))
        img = [img] if img is None else img
        return self.simple_test(img_metas[0], img[0], points[0],points_raw[0],points_gt[0],points_mis[0], global_points[0],z_points[0], **kwargs)
        # return self.simple_test(img_metas[0], img[0], **kwargs)

    def simple_test_pts(self, x, points_raw,points_gt,points_mis,global_points,z_points,img_metas, rescale=False):
        outs = self.pts_bbox_head(x,img_metas ,points_raw,points_gt,points_mis,global_points,z_points)
    ...
    def simple_test(self, img_metas, img=None, points=None, points_raw =None, points_gt=None,points_mis=None,global_points=None,z_points=None,rescale=False):
        world_size = get_dist_info()[1]
        if world_size == 1:  # online
            return self.simple_test_online(img_metas, img, points,points_raw,points_gt,points_mis,global_points,z_points, rescale)
        else:  # offline
    ...
    def simple_test_online(self, img_metas, img=None, points=None, points_raw =None, points_gt=None,points_mis=None,global_points=None, z_points=None,rescale=False):
        ...
        bbox_pts = self.simple_test_pts(img_feats,points_raw,points_gt,points_mis,global_points,z_points,img_metas, rescale=rescale)
        for result_dict, pts_bbox in zip(bbox_list, bbox_pts):
            result_dict['pts_bbox'] = pts_bbox
