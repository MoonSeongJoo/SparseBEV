...
class LoadPointsFromMultiSweeps_moon(object):
    ...
    def visualize_point_cloud(self, points):
        ...
        # print("Points dtype:", points_np.dtype)

        # # Open3D의 PointCloud 객체 생성
        # pcd = o3d.geometry.PointCloud()
        # pcd.points = o3d.utility.Vector3dVector(points_np[:, :3])  # x, y, z 좌표만 사용
        # pcd.paint_uniform_color([0.5, 0.5, 0.5])  # 회색으로 설정
        ...
        # 포인트 클라우드 플로팅
        # ax.scatter(points_np[:, 0], points_np[:, 1], points_np[:, 2], c='gray', s=1)
        ax.scatter(points_np[:, 0], points_np[:, 1], c='gray', s=1)
