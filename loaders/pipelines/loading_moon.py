...
import math
import random
# from mmdet3d.datasets.image_display import display_depth_maps, colormap
import matplotlib.pyplot as plt
# from ...ops.dense_depth_map.src.dense_depth_map import dense_map_gpu_optimized
# from dense_depth_map_cuda.dense_depth_map import dense_map_gpu_optimized
...
class PointToMultiViewDepth(object):
    ...
    def add_mis_calibration(self, extrinsic, intrinsic, points_lidar, post_rots, post_trans, cid, imgs):
        self.max_r = 7.5
        self.max_t = 0.20
        # 회전 각도 랜덤 생성 (deg 단위)
        max_angle = self.max_r  # 최대 회전 각도
        ...
        # intrinsic 변환
        KT = intrinsic.matmul(lidar2img_mis_calibrated)
        ...
        mis_calibrated_depth_map, uv , z = self.points2depthmap(points_img_mis_calibrated, imgs.shape[2], imgs.shape[3])
        
        return mis_calibrated_depth_map, uv , z
    

    # #####
    # def dense_map_gpu_optimized(self,Pts, n, m, grid):
    #     device = Pts.device
    #     ng = 2 * grid + 1
    ...
    def dense_map_gpu_optimized_vectorized(self, Pts, n, m, grid):
        device = Pts.device
        ng = 2 * grid + 1
        ...
        # 커널의 모든 변위를 미리 계산
        displacements = torch.arange(-grid, grid + 1, device=device)
        dX, dY = torch.meshgrid(displacements, displacements)
        dX = dX.reshape(-1)
        dY = dY.reshape(-1)

        # 변위에 따른 인덱스 변화를 계산
        mX_idx_exp = mX_idx[:, None] + dX[None, :]
        mY_idx_exp = mY_idx[:, None] + dY[None, :]

        # 범위를 벗어나는 인덱스 제거
        valid = (mX_idx_exp >= 0) & (mX_idx_exp < m) & (mY_idx_exp >= 0) & (mY_idx_exp < n)
        # 범위를 벗어나는 인덱스 제거 전에 valid 마스크를 1차원으로 변환
        valid_flat = valid.view(-1)
        # 이제 각 텐서에 대해 1차원 valid 마스크를 적용
        mX_idx_exp_flat = mX_idx_exp.view(-1)[valid_flat]
        mY_idx_exp_flat = mY_idx_exp.view(-1)[valid_flat]
        dX_flat = dX.repeat(mX_idx.size(0))[valid_flat]
        dY_flat = dY.repeat(mY_idx.size(0))[valid_flat]

        # 거리 가중치 계산 시에는 flat 버전의 변수 사용
        s = 1 / torch.sqrt(dX_flat ** 2 + dY_flat ** 2)
        s[torch.isinf(s)] = 0

        # 가중치가 적용된 밀도 누적
        Y = torch.zeros((m, n), dtype=torch.float32, device=device)
        S = torch.zeros((m, n), dtype=torch.float32, device=device)
        Y.index_put_((mX_idx_exp_flat, mY_idx_exp_flat), s * mD[mX_idx_exp_flat, mY_idx_exp_flat], accumulate=True)
        S.index_put_((mX_idx_exp_flat, mY_idx_exp_flat), s, accumulate=True)

        S[S == 0] = 1  # 0으로 나누는 것 방지
        out = Y / S
    ...
    def dense_map(self, Pts ,n, m, grid):
        ...
        mX = torch.full((m, n), float('inf'), dtype=torch.float32, device='cpu')
        mY = torch.full((m, n), float('inf'), dtype=torch.float32, device='cpu')
        mD = torch.zeros((m, n), dtype=torch.float32, device='cpu')
        ...
        KmX = torch.zeros((ng, ng, m - ng, n - ng), dtype=torch.float32, device='cpu')
        KmY = torch.zeros((ng, ng, m - ng, n - ng), dtype=torch.float32, device='cpu')
        KmD = torch.zeros((ng, ng, m - ng, n - ng), dtype=torch.float32, device='cpu')
        ...
        S[S == 0] = 1
        out = torch.zeros((m, n), dtype=torch.float32, device='cpu')
        out[grid + 1 : -grid, grid + 1 : -grid] = Y/S
        return out 
    ...
    def __call__(self, results):
        ...
        depth_map_list = []
        mis_calibrated_depth_map_list = []
        dense_depth_map_list = []
        dense_mis_calibrated_depth_map_list = []
        lidar_extrinsic_cal_list = []
        intrinsic_list = []
        post_rots_list = []
        post_trans_list = []
        ...
        for cid in range(len(results['cam_names'])):
            ...
            depth_gt, gt_uv,gt_z = self.points2depthmap(points_img, imgs.shape[2], imgs.shape[3])
            # generation of dense_depth_map  
            lidarOnImage = torch.cat((gt_uv, gt_z.unsqueeze(1)), dim=1)
            # dense_depth_img_gt_cuda = dense_map_gpu_optimized(lidarOnImage.T , imgs.shape[3], imgs.shape[2], 4)
            dense_depth_img_gt = self.dense_map_gpu_optimized(lidarOnImage.T , imgs.shape[3], imgs.shape[2], 4)
            # dense_depth_img_gt1 = self.dense_map_gpu_optimized_vectorized(lidarOnImage.T , imgs.shape[3], imgs.shape[2], 4)
            ...
            # camera_image_rgb = img[:, :, [2, 1, 0]]
            # disp_pc_lidar_np = lidarOnImage.detach().cpu().numpy()
            # plt.figure(figsize=(10, 10))
            # plt.imshow(img)
            # plt.scatter(disp_pc_lidar_np[:, 0], disp_pc_lidar_np[:, 1], c=disp_pc_lidar_np[:, 2], s=0.5)
            # plt.title("input display", fontsize=22)
            # plt.savefig('raw_gt_scatter_lidar2.png')
            # plt.close()   
            
            depth_map_list.append(depth_gt)
            dense_depth_map_list.append(dense_depth_img_color_gt)
            
            # Mis-calibration을 추가하여 mis-calibrated depth map 계산
            depth, uv, z = self.add_mis_calibration(lidar2cam,cam2img, points_lidar, post_rots, post_trans, cid, imgs)
            # generation of dense_depth_map  
            lidarOnImage = torch.cat((uv, z.unsqueeze(1)), dim=1)
            dense_depth_img = self.dense_map_gpu_optimized(lidarOnImage.T , imgs.shape[3], imgs.shape[2], 4)
            dense_depth_img = dense_depth_img.to(dtype=torch.uint8)
            dense_depth_img_color = colormap(dense_depth_img)
            
            mis_calibrated_depth_map_list.append(depth)
            dense_mis_calibrated_depth_map_list.append(dense_depth_img_color)
            lidar_extrinsic_cal_list.append(lidar2cam)
            intrinsic_list.append(cam2img)
            post_rots_list.append(post_rots[cid])
            post_trans_list.append(post_trans[cid])
            ...
            # # 이미지 표시
            # fig, axs = plt.subplots(2, 1, figsize=(20, 10))  # 1행 2열로 서브플롯 설정

            # # 카메라 이미지 표시
            # # axs[0].imshow(camera_image_rgb)
            # axs[0].imshow(depth_gt, alpha=1.0)
            # axs[0].set_title('calibrated depth img')
            # axs[0].axis('off')  # 축 표시 제거

            # # 라이다 뎁스 이미지 표시
            # # axs[1].imshow(camera_image_rgb)
            # axs[1].imshow(depth, alpha=1.0)
            # axs[1].set_title('mis-calibrated depth img')
            # axs[1].axis('off')  # 축 표시 제거
            # plt.savefig('raw_gt_img4.png')
            # plt.close()   
            # print ('display end')
            
        depth_map = torch.stack(depth_map_list)
        dense_depth_map = torch.stack(dense_depth_map_list)
        mis_calibrated_depth_map = torch.stack(mis_calibrated_depth_map_list)  # Mis-calibrated depth maps 스택
        dense_mis_calibrated_depth_map = torch.stack(dense_mis_calibrated_depth_map_list)
        lidar_extrinsic_cal_list = torch.stack(lidar_extrinsic_cal_list)
        intrinsic_list = torch.stack(intrinsic_list)
        post_rots_list = torch.stack(post_rots_list)
        post_trans_list = torch.stack(post_trans_list)
        
        results['intrinsic'] = intrinsic_list
        results['gt_extrinsic'] = lidar_extrinsic_cal_list
        results['post_rots'] = post_rots_list
        results['post_trans'] = post_trans_list
        
        results['gt_depth'] = depth_map
        results['dense_gt_depth'] = dense_depth_map
        results['mis_calibrated_gt_depth'] = mis_calibrated_depth_map  # 결과에 mis-calibrated depth map 추가
        results['dense_mis_calibrated_gt_depth'] = dense_mis_calibrated_depth_map

        
        # # # 모든 카메라 ID에 대해 depth map과 mis-calibrated depth map 디스플레이
        # for cid in range(len(results['cam_names'])):
        #     display_depth_maps(imgs, dense_depth_map, dense_mis_calibrated_depth_map, cid)
...
class LoadAnnotationsBEVDepth(object):
    ...
    def __call__(self, results):
        ...
        gt_labels = torch.tensor(gt_labels_np, dtype=torch.int64)

        # gt_boxes, gt_labels = torch.tensor(gt_boxes_np,dtype=torch.float32), torch.tensor(gt_labels_np,dtype=torch.int64)
